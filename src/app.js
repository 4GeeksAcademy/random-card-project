window.onload = function() {
    ...
    function renderCard() {
      ...
      number = number < 11 ? (number > 1 ? number : (number === 1 ? "A" : "J")) : (number === 11 ? "Q" : "K");
      //2- re-assign innerHTML of suit and number
      icons.forEach(icon => icon.innerHTML = suit);
      suit === "♦" || suit === "♥" ? (icons.forEach(icon => icon.classList.add("isRed"))):(icons.forEach(icon => icon.classList.remove("isRed")));
